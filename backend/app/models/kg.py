...
class KGNode(Base):
    ...
    external_id: Mapped[str] = mapped_column(String(255), unique=True, index=True, nullable=False)  # e.g. "n1"
    ...
    outgoing_edges: Mapped[list["KGEdge"]] = relationship("KGEdge", foreign_keys="KGEdge.source_id", back_populates="source", cascade="all, delete-orphan")
    incoming_edges: Mapped[list["KGEdge"]] = relationship("KGEdge", foreign_keys="KGEdge.target_id", back_populates="target", cascade="all, delete-orphan")
...
class KGEdge(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    source_id: Mapped[int] = mapped_column(Integer, ForeignKey("kg_nodes.id", ondelete="CASCADE"), nullable=False, index=True)
