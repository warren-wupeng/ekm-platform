...
def _mirror_to_neo4j(document_id: int) -> None:
    ...
    async def _push() -> None:
        ...
        for ie in inter_snapshots:
            ...
            edge_props = dict(ie.get("properties") or {})
            if ie.get("confidence") is not None:
                edge_props["confidence"] = ie["confidence"]
            edge_props["needs_review"] = ie.get("needs_review", False)
            if ie.get("edge_id"):
                edge_props["edge_id"] = ie["edge_id"]
            await upsert_relation(
                source_external_id=src["external_id"],
